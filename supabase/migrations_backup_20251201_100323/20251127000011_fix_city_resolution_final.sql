-- =====================================================
-- Migration: Fix city resolution in dashboard favorites (v3 - FINAL)
-- Date: 2025-11-27
-- Problem: CTE approach was too complex, use proven subquery logic
-- Solution: Copy exact logic from get_user_favorite_businesses (3 subqueries)
-- =====================================================

-- Drop existing function
DROP FUNCTION IF EXISTS public.get_client_dashboard_data(UUID, TEXT, TEXT) CASCADE;
-- Recreate function with PROVEN city resolution logic (same as get_user_favorite_businesses)
...
BEGIN
  ...
  -- 3. FAVORITES ✨ WITH CITY RESOLUTION (exact same logic as get_user_favorite_businesses)
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'id', b.id,
      'name', b.name,
...
COMMENT ON FUNCTION public.get_client_dashboard_data(UUID, TEXT, TEXT) IS 
  'Returns consolidated dashboard data for a client including appointments, stats, favorites (with city name resolution using proven subquery logic from get_user_favorite_businesses), suggestions filtered by preferred city/region';
