-- =====================================================
-- Migration: Fix city resolution in dashboard favorites (v2)
-- Date: 2025-11-27
-- Problem: Previous version had inefficient subqueries, favorites return empty
-- Solution: Use LEFT JOIN locations + LEFT JOIN cities for efficient resolution
-- =====================================================
...
-- Recreate function with efficient city name resolution using LEFT JOIN
...
BEGIN
  ...
  -- 3. FAVORITES ✨ WITH EFFICIENT CITY RESOLUTION USING LEFT JOIN
  WITH business_locations AS (
    SELECT DISTINCT ON (bl.business_id)
      bl.business_id,
      CASE 
        WHEN bl.city ~ v_uuid_regex AND ct.name IS NOT NULL
        THEN ct.name
        ELSE bl.city
      END AS city_name
    FROM locations bl
    LEFT JOIN cities ct ON bl.city ~ v_uuid_regex AND ct.id = bl.city::uuid
  ...
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      ...
      'average_rating', b.average_rating,
      'total_reviews', b.total_reviews,
      'city', COALESCE(bl.city_name, 'Sin ubicación')
...
COMMENT ON FUNCTION public.get_client_dashboard_data(UUID, TEXT, TEXT) IS 
  'Returns consolidated dashboard data for a client including appointments, stats, favorites (with efficient city name resolution using LEFT JOIN), suggestions filtered by preferred city/region';
