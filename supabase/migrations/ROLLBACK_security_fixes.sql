-- ============================================================================
-- ROLLBACK SCRIPT: Revertir cambios de seguridad RLS y SECURITY DEFINER
-- Fecha: 21 de octubre, 2025
-- USO: Ejecutar este script SOLO si la app falla después de las migraciones
-- ============================================================================

-- ============================================================================
-- ROLLBACK 1: Deshabilitar RLS en tablas de catálogo
-- ============================================================================
...
ALTER TABLE public.health_insurance DISABLE ROW LEVEL SECURITY;
...
-- ============================================================================
-- ROLLBACK 2: NO RECREAR business_categories_backup
-- (No es necesario revertir - tabla temporal de desarrollo)
-- ============================================================================

-- ============================================================================
-- ROLLBACK 3: Restaurar SECURITY DEFINER en views
-- ============================================================================

-- 1. appointment_details con SECURITY DEFINER
...
-- 2. employee_performance con SECURITY DEFINER
DROP VIEW IF EXISTS public.employee_performance CASCADE;
CREATE VIEW public.employee_performance 
...
-- 3. business_stats con SECURITY DEFINER
DROP VIEW IF EXISTS public.business_stats CASCADE;
CREATE VIEW public.business_stats 
WITH (security_invoker = false)
AS
SELECT 
    b.id as business_id,
...
-- 4. location_services_availability con SECURITY DEFINER
DROP VIEW IF EXISTS public.location_services_availability CASCADE;
CREATE VIEW public.location_services_availability 
WITH (security_invoker = false)
AS
SELECT 
    ls.location_id,
...
GROUP BY 
    ...
    ls.service_id, s.name, s.duration_minutes, s.price, ls.is_available;

-- 5. tax_report_by_period con SECURITY DEFINER
DROP VIEW IF EXISTS public.tax_report_by_period CASCADE;
CREATE VIEW public.tax_report_by_period 
WITH (security_invoker = false)
AS
SELECT 
    t.business_id,
...
-- 6. financial_summary con SECURITY DEFINER
...
-- 7. fiscal_obligations_status con SECURITY DEFINER
DROP VIEW IF EXISTS public.fiscal_obligations_status CASCADE;
CREATE VIEW public.fiscal_obligations_status 
WITH (security_invoker = false)
AS
SELECT 
    t.business_id,
...
-- 8. v_unread_chat_email_stats con SECURITY DEFINER
DROP VIEW IF EXISTS public.v_unread_chat_email_stats CASCADE;
CREATE VIEW public.v_unread_chat_email_stats 
WITH (security_invoker = false)
AS
SELECT 
    cp.user_id,
...
COMMENT ON SCHEMA public IS 'ROLLBACK ejecutado - views restauradas con SECURITY DEFINER';
