...
CREATE OR REPLACE FUNCTION public.get_client_dashboard_data(p_client_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- =====================================================
  -- Obtener ciudad preferida del usuario (si existe)
  SELECT preferred_city INTO v_preferred_city
  FROM profiles
  ...
  FROM (
    SELECT 
      ...
      b.city,
      b.state,
      -- Calcular relevancia: negocios en ciudad preferida primero, luego por rating
      CASE 
        WHEN b.city = v_preferred_city THEN 10
        ELSE 0
