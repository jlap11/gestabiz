-- =====================================================
-- Migration: Add p_preferred_city_id parameter to get_client_dashboard_data RPC
-- Purpose: Fix Edge Function error by supporting optional preferred_city_id
-- =====================================================
...
BEGIN
  ...
  -- =====================================================
  -- 3. SUGGESTIONS (con CTE deduplication y city filter)
  -- =====================================================
  WITH unique_businesses AS (
    ...
    WHERE be.status = 'approved'
      ...
      -- ✅ Filter by city if preferred_city_id is provided
      AND (p_preferred_city_id IS NULL OR EXISTS (
        SELECT 1
        FROM locations loc
  ...
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      ...
      'average_rating', b.average_rating,
      'total_reviews', b.total_reviews,
      'city', COALESCE(bl.location_city, 'Sin ubicación'),
      'state', COALESCE(bl.location_state, ''),
      'country', COALESCE(bl.location_country, 'Colombia')
    )
