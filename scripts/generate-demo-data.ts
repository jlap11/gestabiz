...
async function createBusinesses() {
  ...
  for (const dist of cityDistribution) {
    for (let i = 0; i < dist.count; i++) {
      ...
      const subcategories =
        BUSINESS_CATEGORIES[category as keyof typeof BUSINESS_CATEGORIES].subcategories
...
async function createServices() {
  ...
  for (const business of generatedBusinesses) {
    ...
    const serviceList = categoryData.services[
      business.subcategory as keyof typeof categoryData.services
    ] as string[] | undefined
...
async function assignServicesToEmployees() {
  ...
  for (const employee of generatedEmployees) {
    for (const businessData of employee.businesses) {
      ...
      const businessServices = generatedServices.filter(
        s => s.business_id === businessData.business_id
      )
...
async function createAppointments() {
  ...
  for (const client of activeClients) {
    ...
    for (let i = 0; i < numAppointments; i++) {
      ...
      const employeeServices = generatedServices.filter(
        s => s.business_id === randomBusinessData.business_id
      )
...
async function generateCSVFiles() {
  ...
  const allUsersCSV = [
    ...
    ...generatedUsers.map(
      u => `${u.email},${u.password},${u.full_name},${u.phone},${u.role},${u.id}`
    ),
  ].join('\n')
  ...
  const ownersCSV = [
    ...
    ...owners.map(u => `${u.email},${u.password},${u.full_name},${u.phone},${u.id}`),
  ].join('\n')
  ...
  const clientsCSV = [
    'email,password,full_name,phone,user_id',
    ...clients.map(u => `${u.email},${u.password},${u.full_name},${u.phone},${u.id}`),
  ].join('\n')
