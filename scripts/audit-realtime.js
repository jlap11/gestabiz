...
try {
  const result = execSync('git grep -n "\\.subscribe()" -- "src/**/*.ts" "src/**/*.tsx"', {
    encoding: 'utf8',
  })
...
try {
  const result = execSync(
    'git grep -n "console\\.log.*payload\\|console\\.log.*channel\\|console\\.log.*Subscrib" -- "src/**/*.ts"',
    { encoding: 'utf8' }
  )
  const lines = result.trim().split('\n')
...
try {
  const result = execSync(
    'git grep -n "}, \\[.*fetch.*\\]\\|}, \\[.*subscribe.*\\]" -- "src/**/*.ts" "src/**/*.tsx"',
    { encoding: 'utf8' }
  )
  const lines = result.trim().split('\n')
...
try {
  const result = execSync('git grep -n "supabase\\.channel(" -- "src/**/*.ts"', {
    encoding: 'utf8',
  })
  const lines = result.trim().split('\n')
...
try {
  const files = execSync('git grep -l "setInterval" -- "src/**/*.ts" "src/**/*.tsx"', {
    encoding: 'utf8',
  })
    .trim()
    .split('\n')
...
subscribeFiles.forEach(file => {
  ...
  if (subscribeCount > removeChannelCount) {
    console.log(
      `   ⚠️  ${file}: ${subscribeCount} subscribe pero solo ${removeChannelCount} removeChannel`
    )
    issues.push({
      type: 'missing-remove-channel',
      severity: 'CRITICAL',
      file,
      subscribeCount,
      removeChannelCount,
