...
async function createTransactions() {
  ...
  for (const apt of paidAppointments) {
    ...
    const { error } = await supabase.from('transactions').insert(incomeData)
  ...
  const { data: businesses } = await supabase.from('businesses').select('id, owner_id')
  ...
  const expenseCategories = [
    { category: 'rent', min: 800000, max: 3000000, desc: 'Arriendo de local' },
    {
      ...
      min: 150000,
      max: 500000,
      desc: 'Servicios públicos (luz, agua, gas)',
    },
  ...
  for (const business of businesses) {
    for (const { month, year, name } of months) {
      ...
      for (const expCat of selectedCategories) {
        ...
        const { error } = await supabase.from('transactions').insert(expenseData)
  ...
  // Resumen final
  const { data: summary } = await supabase.from('transactions').select('type')
  ...
  const { data: amountsByType } = await supabase.from('transactions').select('type, amount')

  if (amountsByType) {
    const totals = amountsByType.reduce(
      (acc, t) => {
