...
async function setupTestData() {
  ...
  const { duration: partDuration } = await measureTime(
    ...
    async () => {
      const { error } = await supabase.from('chat_participants').insert(participantInserts)
...
async function generateMessages(conversationId: string, userIds: string[]) {
  ...
  for (let batch = 0; batch < batches; batch++) {
    ...
    const batchEnd = Math.min((batch + 1) * TEST_CONFIG.batchSize, TEST_CONFIG.numMessages)
  ...
  console.log(
    `   Throughput: ${(stats.totalMessages / (stats.totalTime / 1000)).toFixed(2)} msg/s\n`
  )
...
async function testQueryPerformance(conversationId: string) {
  ...
  const { duration: d1 } = await measureTime('Query: Últimos 50 mensajes', async () => {
    const { data, error } = await supabase
      .from('chat_messages')
      .select('*')
  ...
  const { duration: d2 } = await measureTime('Query: TODOS los mensajes (1000+)', async () => {
    const { data, error } = await supabase
      .from('chat_messages')
      .select('*')
      .eq('conversation_id', conversationId)
  ...
  const { duration: d3 } = await measureTime('Query: COUNT total', async () => {
  ...
  const { duration: d4 } = await measureTime('Query: Search con LIKE', async () => {
    const { data, error } = await supabase
      .from('chat_messages')
      .select('*')
      .eq('conversation_id', conversationId)
  ...
  const { duration: d5 } = await measureTime('Query: Con JOIN a profiles (sender)', async () => {
    const { data, error } = await supabase
      .from('chat_messages')
      .select(
        `
...
function analyzeResults(setupStats: any, generateStats: any, queryStats: Record<string, number>) {
...
async function cleanup(conversationId: string) {
  ...
  const { duration } = await measureTime('Eliminar conversación y mensajes', async () => {
