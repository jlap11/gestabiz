...
async function createAppointments() {
  ...
  const { data: employees } = await supabase.from('business_employees').select('employee_id')
  ...
  const clients = allUsers?.filter(u => !employeeIds.has(u.id) && !ownerIds.has(u.id)).slice(0, 70) // Tomar 70 clientes
  ...
  const { data: employeeServices } = await supabase.from('employee_services').select(`
      employee_id,
      service_id,
      business_id,
      location_id,
  ...
  const { data: statusCounts } = await supabase.from('appointments').select('status')
  ...
  const { data: monthCounts } = await supabase.from('appointments').select('start_time')

  if (monthCounts) {
    const months = monthCounts.reduce(
      (acc, apt) => {
        const month = new Date(apt.start_time).toLocaleDateString('es-CO', {
          year: 'numeric',
          month: 'long',
        })
