...
export function useChat(userId: string | null) {
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      ...
      logger.info('[useChat] fetchConversations for userId:', { userId })
      ...
      logger.info('[useChat] participantsData:', { participantsData })
      logger.info('[useChat] participantsError:', { participantsError })

      if (participantsError) throw participantsError

      if (!participantsData || participantsData.length === 0) {
        logger.info('[useChat] No participants found, setting empty conversations')
        setConversations([])
        return
      }
      ...
      if (conversationIds.length > 0) {
        ...
        if (lastMessagesError) {
          logger.error('[useChat] Error fetching last message senders:', { error: lastMessagesError })
        } else if (lastMessagesData) {
          ;(lastMessagesData as Array<{ conversation_id: string; sender_id: string }>).forEach(
            message => {
      ...
      logger.info('[useChat] Final conversationsWithUsers:', conversationsWithLastSender)
      logger.info('[useChat] Setting', conversationsWithLastSender.length, 'conversations')

      setConversations(conversationsWithLastSender as ChatConversation[])
    } catch (err) {
      const error = err as Error
      logger.error('[useChat] Error in fetchConversations:', err)
  ...
  const fetchMessages = useCallback(
    async (conversationId: string, limit = 50) => {
      ...
      try {
        ...
        const error = err as Error
        logger.error('[useChat] Error fetching messages:', err)
        setError(error.message)
      }
    },
    [userId]
  ...
  const fetchTypingIndicators = useCallback(
    async (conversationId: string) => {
      ...
      try {
        ...
        logger.error('Error fetching typing indicators:', err)
      }
    },
    [userId]
  )
  ...
  const sendMessage = useCallback(
    async (params: SendMessageParams) => {
      ...
      try {
        ...
        setMessages(prev => ({
          ...prev,
          [params.conversation_id]: [...(prev[params.conversation_id] || []), optimisticMessage],
        }))

        // Send via RPC
        const { data: messageId, error: rpcError } = await supabase.rpc('send_message', {
  ...
  const markMessagesAsRead = useCallback(
    async (conversationId: string, lastMessageId?: string) => {
      if (!userId) {
        logger.warn('[useChat] ⚠️ markMessagesAsRead called without userId')
        return
      }

      logger.info('[useChat] 🔔 Calling mark_messages_as_read RPC', {
        conversationId,
        userId,
        lastMessageId,
      })
      ...
      try {
        const { data: count, error: rpcError } = await supabase.rpc('mark_messages_as_read', {
        ...
        if (rpcError) {
          logger.error('[useChat] ❌ RPC mark_messages_as_read error:', { error: rpcError })
          throw rpcError
        }

        logger.info('[useChat] ✅ RPC mark_messages_as_read success', { count })
        ...
        try {
          ...
          if (notifError) {
            logger.error('[useChat] ❌ Error clearing chat notifications:', { error: notifError })
          } else {
            logger.info(
              `[useChat] ✅ Cleared ${clearedNotifs?.length || 0} chat notifications for conversation ${conversationId}`
            )
          }
        } catch (notifErr) {
          // No bloquear si falla - logging solo
          logger.error('[useChat] ❌ Failed to clear chat notifications:', { error: notifErr })
        }

        return count
      } catch (err: any) {
        logger.error('[useChat] ❌ Error marking messages as read:', { error: err })
        logger.error('[useChat] ❌ Error details:', {
          message: err?.message,
          code: err?.code,
          details: err?.details,
          hint: err?.hint,
  ...
  const debouncedMarkAsRead = useCallback(
    (conversationId: string, messageId: string) => {
      ...
      markAsReadTimeoutRef.current = setTimeout(() => {
        ...
        if (pending) {
          logger.info('[useChat] ⏱️ Executing debounced mark as read')
  ...
  const updateTypingIndicator = useCallback(
    async (conversationId: string, isTyping: boolean) => {
      ...
      try {
        ...
        logger.error('Error updating typing indicator:', { error: err })
      }
    },
    [userId]
  )
  ...
  const toggleArchiveConversation = useCallback(
    async (conversationId: string, isArchived: boolean) => {
      try {
        ...
        setConversations(prev =>
          prev.map(conv =>
            conv.id === conversationId ? { ...conv, is_archived: isArchived } : conv
          )
        )
      } catch (err: any) {
        logger.error('Error toggling archive:', { error: err })
        throw err
      }
    },
    []
  ...
  const toggleMuteConversation = useCallback(
    async (conversationId: string, isMuted: boolean) => {
      ...
      try {
        ...
        // Update local state
        setConversations(prev =>
          prev.map(conv => (conv.id === conversationId ? { ...conv, is_muted: isMuted } : conv))
        )
      } catch (err: any) {
        logger.error('Error toggling mute:', { error: err })
        throw err
      }
    },
    [userId]
  ...
  const togglePinConversation = useCallback(
    async (conversationId: string, isPinned: boolean) => {
      ...
      try {
        ...
        setConversations(prev =>
          prev.map(conv => (conv.id === conversationId ? { ...conv, is_pinned: isPinned } : conv))
        )
      } catch (err: any) {
        logger.error('Error toggling pin:', { error: err })
        throw err
      }
    },
    [userId]
  ...
  useEffect(() => {
    if (activeConversationId && userId) {
      logger.info('[useChat] 📥 Auto-loading messages for conversation:', { conversationId: activeConversationId })
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        payload => {
          logger.info('[useChat] 📡 Participant updated:', { payload })
  ...
  useEffect(() => {
    ...
    logger.info('[useChat] 🔔 Setting up realtime subscription for conversation:', { conversationId: activeConversationId })
    ...
    const messagesChannel = supabase
      ...
      .on(
        ...
        async payload => {
          logger.info('[useChat] 📨 New message received:', { message: payload.new })
          ...
          logger.info('[useChat] 📨 Full message data:', { message: newMessage })
          ...
          if (newMessage) {
            ...
            if (mappedMessage.sender_id === userId) {
              logger.info('[useChat] ⏭️ Skipping own message (already added optimistically)')
              return
            }
            ...
            setMessages(prev => {
              ...
              if (messageExists) {
                logger.info('[useChat] ⚠️ Message already exists, skipping duplicate')
                return prev
              }

              logger.info('[useChat] ✅ Adding new message from other user to state')
              return {
                ...prev,
                [activeConversationId]: [...existingMessages, mappedMessage],
              }
            })

            // Mark as read SOLO si el mensaje es de otro usuario (con debounce)
            if (mappedMessage.sender_id !== userId) {
              logger.info('[useChat] 👀 Scheduling debounced mark as read')
      ...
      .on(
        ...
        payload => {
          logger.info('[useChat] ✏️ Message updated:', { message: payload.new })

          setMessages(prev => ({
            ...prev,
            [activeConversationId]:
              prev[activeConversationId]?.map(msg =>
      ...
      .subscribe(status => {
        logger.info('[useChat] 📡 Messages channel status:', { status })
      })
