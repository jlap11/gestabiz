...
export function useEmployeeServices(filters?: EmployeeServiceFilters) {
  ...
  const fetchEmployeeServices = async () => {
    try {
      ...
      let query = supabase.from('employee_services').select(`
          *,
      ...
      if (filters?.min_expertise_level)
        query = query.gte('expertise_level', filters.min_expertise_level)
  ...
  const getEmployeesForService = async (
    serviceId: string,
    locationId: string
