...
export function useNotifications() {
  ...
  const scheduleReminder = (appointment: Appointment) => {
    ...
    const appointmentTime = appointment.startTime || appointment.time || appointment.start_time?.split('T')[1]?.substring(0, 5)
      ...
      const updatedQueue = current.map(notification => {
        if (!notification.sent && new Date(notification.scheduledFor) <= now) {
          ...
          if (appointment && appointment.status === 'scheduled') {
            ...
            const timeUntil = Math.round((new Date(`${appointmentDate}T${appointmentTime}`).getTime() - now.getTime()) / (1000 * 60))
