...
export function useFinancialReports(): UseFinancialReportsReturn {
  ...
  const generateProfitAndLoss = useCallback(
    async (filters: ReportFilters): Promise<ProfitAndLossReport> => {
      try {
        ...
        let query = supabase.from('transactions').select('*').eq('business_id', filters.business_id)
        ...
        const average_transaction =
          transaction_count > 0 ? (total_income + total_expenses) / transaction_count : 0
  ...
  // Exportar a CSV
  const exportToCSV = useCallback((data: unknown[], filename: string, options?: ExportOptions) => {
  ...
  const exportToPDF = useCallback(
    (report: ProfitAndLossReport, businessName: string, filename?: string) => {
      ...
      doc.setFont('helvetica', 'bold')
      doc.text(
        `Total Ingresos: $${report.total_income.toLocaleString('es-CO', { minimumFractionDigits: 2 })}`,
        14,
        yPos
      )
      yPos += 15
      ...
      doc.setFontSize(12)
      doc.text(
        `Total Egresos: $${report.total_expenses.toLocaleString('es-CO', { minimumFractionDigits: 2 })}`,
        14,
        yPos
      )
      yPos += 10
      ...
      doc.text(
        `${netLabel}: $${Math.abs(netAmount).toLocaleString('es-CO', { minimumFractionDigits: 2 })}`,
        14,
        yPos
      )
