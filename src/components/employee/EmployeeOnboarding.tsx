...
export function EmployeeOnboarding({ user, onRequestCreated }: EmployeeOnboardingProps) {
  ...
  const handleQRScanned = (data: BusinessInvitationQRData) => {
    // Extract invitation code from QR data
  ...
  return (
    <div className="min-h-screen bg-[#1a1a1a] p-6">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {/* Pending/Approved Requests Info */}
          ...
          <Alert>
            ...
            <AlertDescription>
              {pendingRequests.length > 0 && (
                <p>
                  Tienes {pendingRequests.length} solicitud(es) pendiente(s). El administrador revisará tu solicitud
                  pronto.
                </p>
              )}
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            {/* QR Scanner Button (Desktop: show info, Mobile: open camera) */}
            <Button
              type="button"
              variant="outline"
              className="w-full"
            ...
            {/* QR Scanner Info (Desktop) */}
            {showScanner && (
              <Alert>
                <Camera className="h-4 w-4" />
                <AlertTitle>Escanear QR desde móvil</AlertTitle>
                <AlertDescription>
                  El escaneo de códigos QR está disponible en la aplicación móvil. Por ahora, ingresa el código
                  manualmente.
                </AlertDescription>
