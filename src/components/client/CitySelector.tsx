...
export function CitySelector({
  ...
  return (
    <div className="inline-flex items-center gap-2 px-3 py-2 text-sm" role="toolbar" aria-label={t('citySelector.selectorLabel')}>
       ...
       <div className="inline-flex items-center gap-2">
         <div className="relative inline-block" ref={regionMenuRef}>
           <button
             ...
             aria-label={t('citySelector.selectRegion')}
             title={t('citySelector.selectRegion')}
             aria-haspopup="menu"
           ...
           {regionMenuOpen && (
             <div className="absolute top-full left-0 mt-1 bg-card border border-border rounded-md shadow-lg z-50 min-w-[200px] max-h-[300px] overflow-y-auto" role="listbox" aria-label={t('citySelector.regionList')}>
               {regionsLocal.length === 0 ? (
                 <div className="px-3 py-2 text-sm text-muted-foreground">
                   {regionsLoading ? t('citySelector.loading') : t('citySelector.noRegions')}
                   <button onClick={reloadRegions} className="ml-2 underline text-primary text-xs min-h-[44px] min-w-[44px]" aria-label={t('citySelector.retry')} title={t('citySelector.retry')}>
           ...
           <>
             ...
             <div className="relative inline-block" ref={cityMenuRef}>
               <button
                 ...
                 className="text-sm text-muted-foreground hover:text-foreground transition-colors inline-flex items-center gap-1 min-h-[44px]"
                 aria-label={t('citySelector.selectCity')}
                 title={t('citySelector.selectCity')}
                 aria-haspopup="menu"
               ...
               {cityMenuOpen && (
                 <div className="absolute top-full left-0 mt-1 bg-card border border-border rounded-md shadow-lg z-50 min-w-[200px] max-h-[300px] overflow-y-auto" role="listbox" aria-label={t('citySelector.cityList')}>
                   <button
                     onClick={e => {
                       e.stopPropagation()
                       const region = regionsLocal.find(r => r.id === selectedRegion)
                   ...
                   {citiesLoading ? (
                     ...
                     hookCities.map(city => (
                       <button
                        ...
                        role="option"
                        aria-selected={preferredCityId === city.id}
                        >
                         {city.name}
                       </button>
                     ))
