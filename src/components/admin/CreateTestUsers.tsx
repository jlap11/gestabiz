...
export function CreateTestUsers() {
  ...
  const getRoleStats = () => {
    ...
    const stats = result.users.reduce((acc, user) => {
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          Crea usuarios de ejemplo usando Supabase Edge Function (10 admin, 10 empleados, 10 clientes)
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        {!result && !error && (
          <div className="space-y-4">
            <Alert>
              <AlertDescription>
                ⚠️ <strong>Importante:</strong> Esta acción creará 30 usuarios reales en tu base de datos de Supabase.
                Los usuarios tendrán contraseña temporal: <code>TestPassword123!</code>
              </AlertDescription>
            </Alert>
              ...
              <div>
                ...
                <ul className="text-xs text-orange-600 space-y-1 max-h-32 overflow-y-auto bg-orange-50 p-2 rounded">
                  {result.errors.map((error) => (
                    <li key={error.email}>• {error.email}: {error.error}</li>
                  ))}
                </ul>
              </div>
            )}
