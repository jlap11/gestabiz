...
const isDemoMode = demoFlag || import.meta.env.VITE_DEMO_MODE === 'true' || !hasValidCredentials
...
function createMockClient() {
  ...
  function stubSingleOk() {
    return Promise.resolve({
      data: null,
      error: null as unknown as { code: string; message: string },
    })
  }
  function stubSingleNotFound() {
    return Promise.resolve({ data: null, error: { code: 'PGRST116', message: 'No rows found' } })
  }
  function stubSelect() {
    return { single: stubSingleOk }
  }
  function stubEqUpdate() {
    return { select: stubSelect }
  }
  function makeQuery() {
    const api = {
  ...
  const client = {
    auth: {
      ...
      signUp: () =>
        Promise.resolve({ data: { user: mockUser, session: mockSession }, error: null }),
      signInWithOAuth: () =>
        Promise.resolve({ data: { provider: 'google', url: 'mock-url' }, error: null }),
