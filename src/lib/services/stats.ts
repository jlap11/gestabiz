...
export const statsService = {
  async getDashboardStats(q: DashboardStatsQuery = {}): Promise<DashboardStats> {
    const now = new Date()
    const startDate =
      q.dateRange?.start || new Date(now.getFullYear(), now.getMonth(), 1).toISOString()
    ...
    const initStats = (): DashboardStats => ({
      ...
      scheduled_appointments: list.filter(a => a.status === 'scheduled' || a.status === 'confirmed')
        .length,
    ...
    stats.average_appointment_value =
      completedWithPrice.length > 0 ? stats.revenue_total / completedWithPrice.length : 0
    ...
    const computeTimeWindows = () => {
      ...
      stats.upcoming_today = list.filter(
        a => a.start_time >= startOfToday && a.start_time <= endOfToday
      ).length
      stats.upcoming_week = list.filter(
        a => a.start_time > now.toISOString() && a.start_time <= weekAhead
      ).length
    }
    ...
    const fetchNames = async () => {
      ...
      if (serviceIds.length) {
        ...
        serviceNames = new Map(
          (data || []).map((s: { id: string; name: string }) => [s.id, s.name])
        )
      }
      ...
      if (employeeIds.length) {
        ...
        employeeNames = new Map(
          (data || []).map((p: { id: string; full_name?: string }) => [p.id, p.full_name || p.id])
        )
      }
      ...
      if (locationIds.length) {
        ...
        locationNames = new Map(
          (data || []).map((l: { id: string; name: string }) => [l.id, l.name])
        )
      }
    ...
    stats.popular_services = Array.from(byService.entries())
      ...
      .slice(0, 10)
      .map(([serviceId, v]) => ({
        service: serviceNames.get(serviceId) || serviceId,
        count: v.count,
        revenue: v.revenue,
      }))
